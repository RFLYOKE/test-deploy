...
import BankMitraForm from "../formModal/bankmitra-form";
...
export default function BankMitraPage() {
  ...
  const [form, setForm] = useState<Partial<CabangBankMitra>>({ status: true });
  const [editingId, setEditingId] = useState<number | null>(null);
  const [page, setPage] = useState(1);
  const { isOpen, openModal, closeModal } = useModal();

  const { data, isLoading, refetch } = useGetCabangBankMitrasQuery({
    page,
    paginate: 10,
  });

  const [createCabangBankMitra] = useCreateCabangBankMitraMutation();
  const [updateCabangBankMitra] = useUpdateCabangBankMitraMutation();
  const [deleteCabangBankMitra] = useDeleteCabangBankMitraMutation();

  const mitras = data?.data || [];
  ...
  const handleSubmit = async () => {
    try {
      if (editingId !== null) {
        await updateCabangBankMitra({ id: editingId, payload: form }).unwrap();
        Swal.fire("Berhasil", "Data berhasil diperbarui", "success");
      } else {
        await createCabangBankMitra(form).unwrap();
  ...
  const handleEdit = (c: CabangBankMitra) => {
    setForm(c);
    setEditingId(c.id);
    openModal();
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    if (result.isConfirmed) {
      try {
        await deleteCabangBankMitra(id).unwrap();
  ...
  const filtered = mitras.filter((c) => {
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-bold">Cabang Bank Mitra</h1>
